...
-- F
SELECT CUSTOMER_ID, AMOUNT, AMOUNT/1.06 AS SUBTOTAL
FROM INVOICE;

----------------------------------------------
-- THURSDAY PAIR PROGRAMMING EXERCISES
----------------------------------------------

-- Question 1
CREATE OR REPLACE FUNCTION SUBTOTAL(TOTAL IN INVOICE.AMOUNT%TYPE)
RETURN INVOICE.AMOUNT%TYPE
IS
BEGIN
	RETURN (TOTAL)/1.06;
END;
...
-- Question 2
CREATE OR REPLACE FUNCTION REPEAT_STRING(STR IN VARCHAR2, N IN NUMBER)
RETURN VARCHAR2
IS
REPEATED_STRING VARCHAR2(1000);
I NUMBER := N - 1;
BEGIN
    REPEATED_STRING := STR;
    LOOP
        REPEATED_STRING := REPEATED_STRING || ' ' || STR;
        I := I - 1;
        EXIT WHEN I <= 0;
    END LOOP;
...
BEGIN
    DBMS_OUTPUT.PUT_LINE(REPEAT_STRING('CAT', 5));
END;
...
CREATE OR REPLACE PROCEDURE ENTER_NEW_INVOICE(D INVOICE_DATE%TYPE)
IS
BEGIN
   DELETE (SELECT * FROM INVOICE WHERE CUSTOMER_ID IS NULL);
END;
